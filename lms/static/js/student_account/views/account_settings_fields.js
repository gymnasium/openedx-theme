    ...
    var AccountSettingsFieldViews = {
      ...
      TimeZoneFieldView: FieldViews.DropdownFieldView.extend({
        ...
        updateValueInField: function () {
          ...
          if (this.modelValue()) {
            options = [
              [this.modelValue(), this.displayValue(this.modelValue())],
            ];
      ...
      PasswordFieldView: FieldViews.LinkFieldView.extend({
        ...
        resetPassword: function () {
          var data = {};
          data[this.options.emailAttribute] = this.model.get(
        ...
        successMessage: function () {
          return HtmlUtils.joinHtml(
            ...
            HtmlUtils.interpolateHtml(
              gettext(
                "We've sent a message to {email}. Click the link in the message to reset your password. Didn't receive the message? Contact {anchorStart}technical support{anchorEnd}."
              ), // eslint-disable-line max-len
              {
                ...
                anchorStart: HtmlUtils.HTML(
                  StringUtils.interpolate(
                    ...
                    {
                      passwordResetSupportUrl: this.options
      ...
      SocialLinkTextFieldView: FieldViews.TextFieldView.extend({
        render: function () {
          HtmlUtils.setHtml(
            this.$el,
            HtmlUtils.template(field_text_account_template)({
      ...
      ExtendedFieldTextFieldView: FieldViews.TextFieldView.extend({
        ...
        modelValue: function () {
          var extendedProfileFields = this.model.get(
            this.options.valueAttribute
          );
          for (var i = 0; i < extendedProfileFields.length; i++) {
            // eslint-disable-line vars-on-top
            if (
              extendedProfileFields[i].field_name === this.options.fieldName
            ) {
      ...
      AuthFieldView: FieldViews.LinkFieldView.extend({
        ...
        className: function () {
          return (
            "u-field u-field-social u-field-" + this.options.valueAttribute
          );
        },
        initialize: function (options) {
          this.options = _.extend({}, options);
        ...
        render: function () {
          ...
          HtmlUtils.setHtml(
            this.$el,
            HtmlUtils.template(this.fieldTemplate)({
