(function (define, undefined) {
  ...
  define("gymnasium/js/student_account/views/account_settings_factory", [
    ...
    "gymnasium/js/student_account/views/account_settings_fields",
    "gymnasium/js/student_account/views/account_settings_view",
    ...
    return function (
      ...
      if (syncLearnerProfileData && enterpriseName) {
        ...
        aboutSectionMessage = HtmlUtils.interpolateHtml(
          gettext(
            "Your profile settings are managed by {enterprise_name}. Contact your administrator or {link_start}edX Support{link_end} for help."
          ), // eslint-disable-line max-len
          {
            enterprise_name: enterpriseName,
            link_start: HtmlUtils.HTML(
              StringUtils.interpolate('<a href="{edx_support_url}">', {
      ...
      if (
        !allowEmailChange ||
        (syncLearnerProfileData &&
          enterpriseReadonlyAccountFields.fields.indexOf("email") !== -1)
      ) {
        // eslint-disable-line max-len
        emailFieldView = {
      ...
      if (
        syncLearnerProfileData &&
        enterpriseReadonlyAccountFields.fields.indexOf("name") !== -1
      ) {
        fullnameFieldView = {
      ...
      if (
        syncLearnerProfileData &&
        enterpriseReadonlyAccountFields.fields.indexOf("country") !== -1
      ) {
          ...
          fields: [
            {
              view: new AccountSettingsFieldViews.ReadonlyFieldView({
                ...
                helpMessage: StringUtils.interpolate(
                  gettext(
                    "The name that identifies you on {platform_name}. You cannot change your username."
                  ), // eslint-disable-line max-len
                  { platform_name: platformName }
                ),
              }),
            ...
            },
            {
              view: new AccountSettingsFieldViews.LanguagePreferenceFieldView({
                ...
                helpMessage: StringUtils.interpolate(
                  gettext(
                    "The language used throughout this site. This site is currently available in a limited number of languages. Changing the value of this field will cause the page to refresh."
                  ), // eslint-disable-line max-len
                  { platform_name: platformName }
                ),
      ...
      for (var field in extendedProfileFields) {
